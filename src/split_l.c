 ...
 * Pick the two that are separated most along any dimension, or
 * overlap least.
 * Distance for separation or overlap is measured modulo the width
 * of the space covered by the entire set along that dimension.
 */
 ...
 * Process one rect at a time, using the following hierarchy
 * of criteria.
 *
 * In case of a tie, go to the next test.
 * 1) If one group already has the max number of elements that
 *    will allow the minimum fill for the other group, put r in
 *    the other.
 * 2) Put r in the group whose cover will expand less.  This
 *    automatically takes care of the case where one group cover
 *    contains r.
 * 3) Put r in the group whose cover will be smaller.  This takes
 *    care of the case where r is contained in both covers.
 ...
 * Then put other rects in whichever group will be smallest
 * after addition.
 */
...
void RTreeSplitNode(struct Node *n, struct Branch *b, struct Node **nn)
{
    ...
    /**
     * Note: can't use MINFILL(n) below since n was cleared
     * by GetBranches()
     */
    ...
    /**
     * put branches from buffer in 2 nodes according to
     * chosen partition
     */
    *nn = RTreeNewNode();
...
/**
 ** $Log: split_l.c,v $
 ** Revision 1.4  2002/07/13 19:18:39  andrew
 ...
 **
 ** Revision 1.1  2002/07/10 03:02:58  andrew
